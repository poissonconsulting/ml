...
test_that("ml_analyse", {
  ...
  expect_equal(object, structure(list(expr = "sum(dpois(x, exp(log_lambda), log = TRUE))", 
    pars = list(log_lambda = 0), data = list(x = c(1L, 0L, 2L, 
    2L, 1L, 1L, 2L, 1L, 2L, 1L)), optim = list(par = c(log_lambda = 0.262364097539851), 
...
test_that("ml_analyse two parameters", {
  ...
  expect_equal(object, structure(list(expr = "sum(dnorm(x, par[1], exp(par[2]), log = TRUE))", 
    pars = list(par = c(0, 0)), data = list(x = c(1.51094526422692, 
    2.82869278312871, 0.987584234066254, 2.32153918856514, 2.4661538259704, 
    3.76094943134405, 2.92818478343895, 1.83089852786868, 3.37554243426907, 
    1.66511095305911)), optim = list(par = c(`par[1]` = 2.36756017450177, 
    `par[2]` = -0.184380313221117), value = -12.3455805163068, 
        counts = c(`function` = 34L, gradient = 15L), convergence = 0L, 
        message = NULL, hessian = structure(c(-14.4594144209087, 
...
test_that("ml_analyse fixed parameter", {
  ...
  expect_equal(object, structure(list(expr = "sum(dnorm(x, par[1], exp(par[2]), log = TRUE))", 
    pars = list(par = c(NA, 0)), data = list(x = c(1.51094526422692, 
    2.82869278312871, 0.987584234066254, 2.32153918856514, 2.4661538259704, 
    3.76094943134405, 2.92818478343895, 1.83089852786868, 3.37554243426907, 
    1.66511095305911), par = c(1.5, NA)), optim = list(par = c(`par[2]` = 0.183794587081123), 
