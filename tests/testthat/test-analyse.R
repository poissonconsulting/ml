...
test_that("ml_analyse", {
  ...
  expect_equal(object, structure(list(expr = "sum(dpois(x, exp(log_lambda), log = TRUE))", 
    ...
    optim = list(par = c(log_lambda = 0.262364097539851), value = -12.3618532841626, 
        counts = c(`function` = 19L, gradient = 6L), convergence = 0L, 
        message = NULL, hessian = structure(-13.0000021631993, .Dim = c(1L, 
...
test_that("ml_analyse two parameters", {
  ...
  expect_equal(object, structure(list(expr = "sum(dnorm(x, par[1], exp(par[2]), log = TRUE))", 
    pars = structure(list(par = c(0, 0)), class = "nlist"), data = list(
        x = c(1.51094526422692, 2.82869278312871, 0.987584234066254, 
...
test_that("ml_analyse fixed", {
  ...
  expect_equal(object, structure(list(expr = "sum(dnorm(x, par[1], exp(par[2]), log = TRUE))", 
    ...
    data = list(x = c(-1.36119417607418, 2.57596225921064, -2.92488844854881, 
    1.0606924447185, 1.49277100463189, 5.3613518799151, 2.87322373279497, 
    -0.405240146286015, 4.20983566229993, -0.900579054300623)), 
    optim = list(par = c(`par[2]` = 1.01469510312161), value = -24.3363282753209, 
        counts = c(`function` = 14L, gradient = 7L), convergence = 0L, 
        message = NULL, hessian = structure(-19.999994314901, .Dim = c(1L, 
